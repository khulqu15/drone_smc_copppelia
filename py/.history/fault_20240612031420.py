...
def detect_motor_faults(fault_mask, data):
    motor_faults = {1: False, 2: False, 3: False, 4: False}
    motor_influence = {
        1: ['Actual X', 'Actual Roll'],
        2: ['Actual Y', 'Actual Pitch'],
        3: ['Actual Z', 'Actual Yaw'],
        4: ['Actual X', 'Actual Yaw']
    }

    for motor, influences in motor_influence.items():
        for influence in influences:
            if influence in data.columns and influence in fault_mask.columns:
                aligned_fault_mask = fault_mask[influence].reindex(data.index, fill_value=False)
                if any(aligned_fault_mask):
                    motor_faults[motor] = True
                    data_points = data.loc[aligned_fault_mask, influence]
                    plt.scatter(data_points.index, data_points, color='red', label=f'Motor {motor} Fault', alpha=0.5, edgecolor='black')
...
def visualize_data(data, fault_mask):
    ...
    for i, col in enumerate(columns):
        ...
        plt.scatter(data.index[aligned_fault_mask[actual_col]], data[actual_col][aligned_fault_mask[actual_col]], color='lightgray', label='General Faults')
    ...
    motor_faults = detect_motor_faults(fault_mask, data)
    plt.tight_layout()
    plt.show()
    return motor_faults
...
def main():
    ...
    if data is not None:
        ...
        motor_faults = visualize_data(data, fault_mask)
        print("Detected Motor Faults:", motor_faults)
