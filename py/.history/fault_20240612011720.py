...
def preprocess_data(data):
    ...
    for column in columns:
        noise = np.random.normal(0, 3* data[column].std(), data[column].shape)
...
def simulate_motor_health(data, residuals):
    # Meningkatkan faktor noise untuk membuatnya lebih signifikan
    noise_factor = 0.1  # Menyesuaikan ini untuk melihat pengaruhnya
    motor_weights = np.random.rand(4, residuals.shape[1]) * noise_factor
    print("Random Weights Before Normalization:", motor_weights)

    # Menerapkan normalisasi yang berbeda untuk bobot
    motor_weights /= np.sum(motor_weights, axis=0)  # Menjaga total setiap kolom = 1
    print("Normalized Weights:", motor_weights)

    motor_health = {}
    for i in range(1, 5):
        weighted_error = np.abs(residuals) * motor_weights[i-1]
        motor_health['Motor' + str(i)] = np.nanmean(weighted_error, axis=0)
        print(f"Health for Motor {i}:", motor_health['Motor' + str(i)])

    # Menambahkan baseline noise ke kesehatan motor yang sehat untuk membuatnya lebih realistis
