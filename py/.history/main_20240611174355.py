...
def main():
    ...
    if data is not None:
        ...
        # Membuat plot untuk setiap pasangan variabel yang diinginkan
        fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(12, 8))

        # Mapping dari variabel ke indeks
        labels = ['Roll', 'Pitch', 'Yaw', 'Z', 'Actual Roll', 'Actual Pitch', 'Actual Yaw', 'Actual Z']
        pairs = [('Roll', 'Actual Roll'), ('Pitch', 'Actual Pitch'), ('Yaw', 'Actual Yaw'), ('Z', 'Actual Z')]

        # Plot setiap pasangan di subplot yang sesuai
        for i, (var, actual) in enumerate(pairs):
            ax = axes[i//2, i%2]
            irf.plot(response=actual, impulse=var, ax=ax, plot_stderr=False)
            ax.set_title(f'{var} to {actual}')
            ax.legend([f'{var} to {actual}'])
