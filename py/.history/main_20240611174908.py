...
def main():
    ...
    if data is not None:
        ...
        for i, var in enumerate(variables):
            irf_data = irf.impulse_response(steps=10)
            ax = axes[i//2, i%2]
            ax.plot(irf_data[:, results.names.index(var), results.names.index('Actual ' + var)], label=f'Actual {var}')
            ax.set_title(f'IRF of {var} to Actual {var}')
