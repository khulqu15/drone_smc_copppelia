...
def detect_motor_faults(fault_mask, data_columns):
...
def visualize_data(data, fault_mask, motor_faults):
    ...
    fault_motors = {1: [], 2: [], 3: [], 4: []}

    # Prepare fault motor annotations based on columns affected
    if motor_faults is not None:
        for motor, affected_columns in motor_faults.items():
            for col in affected_columns:
                if col in columns:
                    fault_motors[motor].append(col)

    for i, col in enumerate(columns):
        plt.subplot(2, 3, i+1)
        actual_col = 'Actual ' + col
        plt.plot(data[actual_col], label='Actual')
        # Ensure fault_mask aligns with data index
        ...
        # Annotate suspected motor faults
        if motor_faults is not None:
            fault_text = ", ".join([f"Motor {m}" for m, cols in fault_motors.items() if col in cols])
            if fault_text:
                plt.annotate(f"Fault: {fault_text}", xy=(0.5, 0.9), xycoords='axes fraction', ha='center', va='center', fontsize=12, color='red')
...
def main():
    ...
    if data is not None:
        ...
        visualize_data(stationary_data, fault_mask, None)

        motor_faults = detect_motor_faults(fault_mask, data_columns)
