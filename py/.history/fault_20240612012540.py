...
def simulate_motor_health(data, residuals):
    ...
    motor_health = {}
    for i in range(1, 5):
        weighted_health = np.abs(residuals * motor_weights[i-1]) * np.random.uniform(0, 1)
        motor_health['Motor' + str(i)] = np.nanmean(weighted_health, axis=1)
