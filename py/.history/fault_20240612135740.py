...
def generate_statistics(data, motor_faults):
    stats = {
        'Motor': [],
        'State': [],
        'φ (deg)': [],
    ...
    time_step = 1 / data.index.freqstr  # Assuming data has a time frequency set

    for motor, faults in motor_faults.items():
        is_faulty = faults.any()
...
def main():
    ...
    if data is not None:
        ...
        statistics = generate_statistics(stationary_data, motor_faults)
        print(statistics)
