...
def generate_drone_dataset(data):
    data['mm Size'] = (data['Actual Roll'].abs() + data['Actual Pitch'].abs()) * 1000
    data['φ'] = data['Roll']
    data['θ'] = data['Pitch']
    data['p = φ˙'] = data['Actual Roll']  # assuming Roll rate derivative
    data['q = ˙θ'] = data['Actual Pitch']  # assuming Pitch rate derivative
    data['ze'] = data['Z']
    data['RPM'] = (data['SMC X'].abs() + data['SMC Y'].abs()) * 1000  # Simplified RPM calculation
    
    result_columns = ['mm Size', 'φ', 'θ', 'p = φ˙', 'q = ˙θ', 'ze', 'RPM']
...
def main():
    ...
    if data is not None:
        ...
        df = generate_drone_dataset(data)
        print(df.head())
