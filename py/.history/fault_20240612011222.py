...
def preprocess_data(data):
    ...
    for column in columns:
        noise = np.random.normal(0, 0.1 * data[column].std(), data[column].shape)
...
def simulate_motor_health(data, residuals):
    motor_weights = np.random.rand(4, residuals.shape[1])  # Bobot acak untuk simulasi
