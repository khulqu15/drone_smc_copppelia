...
def simulate_motor_health(data, residuals):
    ...
    for i in range(1, 5):
        ...
        noise = np.random.normal(0, 0.0001, size=weighted_health.shape)  # Make sure noise matches the shape of weighted_health
        motor_health['Motor' + str(i)] = (weighted_health + noise).mean(axis=0)  # Calculate mean health, adding noise correctly
    
    healthy_scale = 0.025 * np.nanmean(residuals.abs(), axis=1)
    healthy_noise = np.random.normal(0, healthy_scale, size=residuals.shape[0])
    motor_health['Motor' + str(i)] = np.mean(np.mean(weighted_health + noise, axis=1))
...
def plot_motor_health(data, motor_health):
    ...
    for i, angle in enumerate(angles):
        ...
        for key, values in motor_health.items():
            ax.hlines(y=values, xmin=min_angle, xmax=max_angle, label=key, color=np.random.rand(3,))
