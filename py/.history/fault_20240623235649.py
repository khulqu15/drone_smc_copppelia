...
def generate_drone_dataset(num_entries=100):
    """
    Generate a dataset for drone rotor simulations with RPM values.

    Parameters:
        num_entries (int): Number of entries to generate in the dataset.

    Returns:
        DataFrame: A pandas DataFrame containing the drone simulation data including motor RPM.
    """
    # Define the simulation parameters
    np.random.seed(42)  # for reproducibility
    mm_size = np.random.randint(1000, 1100, size=num_entries)  # Rotor size between 1000 mm and 1100 mm
    phi = np.random.randint(-5, 5, size=num_entries)  # Phi angle between -5 and 5 degrees
    theta = np.random.randint(-5, 5, size=num_entries)  # Theta angle between -5 and 5 degrees
    phi_dot = np.random.uniform(-35, 35, size=num_entries)  # Phi velocity between -35 and 35 degrees/s
    theta_dot = np.random.uniform(-35, 35, size=num_entries)  # Theta velocity between -35 and 35 degrees/s
    ze = np.random.uniform(3.0, 4.0, size=num_entries)  # Elevation between 3.0 and 4.0 meters
    rpm = np.random.randint(1000, 3000, size=num_entries)  # Motor RPM between 1000 and 3000
