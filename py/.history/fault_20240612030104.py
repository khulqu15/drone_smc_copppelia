...
def detect_faults_from_residuals(residuals):
    ...
    motor_mapping = {
        ...
        'Actual Z': 'Motor 3',
        'Actual Roll': 'Motor 4',
        'Actual Pitch': 'Motor 5',
        'Actual Yaw': 'Motor 6'
    }
    for column in residuals.columns:
        if column in motor_mapping:
            motor_faults = fault_mask[column].any()
            if motor_faults:
                fault_motors[motor_mapping[column]] = residuals[column][fault_mask[column]].abs()
    return fault_motors
...
def visualize_motor_faults(data, fault_motors):
    fig, ax = plt.subplots(figsize=(12, 8))
    for motor, faults in fault_motors.items():
        if not faults.empty:
            ax.plot(data.index, data[motor], label=motor, alpha=0.7)
            ax.scatter(faults.index, data.loc[faults.index, motor], color='red', label=f'Fault in {motor}')
    ax.set_title('Motor Fault Detection')
    ax.set_xlabel('Time')
...
def main():
    ...
    if data is not None:
        ...
        fault_motors = detect_faults_from_residuals(residuals)
        visualize_motor_faults(stationary_data, fault_motors)
