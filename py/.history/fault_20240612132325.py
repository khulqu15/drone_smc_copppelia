...
def generate_fault_dataset(data, motor_faults):
    fault_data = pd.DataFrame()

    for motor, faults in motor_faults.items():
        for column in faults.columns:
            fault_indices = faults[column][faults[column]].index
            df = pd.DataFrame({
                'Time': data.index[fault_indices],
                'Motor': motor,
                'Variable': column,
                'Value': data.loc[fault_indices, column]
            })
            fault_data = pd.concat([fault_data, df], ignore_index=True)
