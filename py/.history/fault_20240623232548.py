...
def generate_statistics(data, motor_faults, time_step=None):
    ...
    for motor, faults in motor_faults.items():
        ...
        φ = data['Roll'].mean()
        θ = data['Pitch'].mean()
        ze = data['Altitude'].mean() if 'Altitude' in data.columns else 0  # Placeholder if altitude data is missing
        ...
        for param in parameters:
            mean_value = data[f'Actual {param}'].mean()
            std_dev = data[f'Actual {param}'].std()
            if param == 'Roll':
                roll_changes = np.diff(data[f'Actual {param}'].dropna()) / time_step
                rpm = np.mean(np.abs(roll_changes)) / 360 * 60
            ...
            stats['RPM'].append(rpm if param == 'Roll' else np.nan)
