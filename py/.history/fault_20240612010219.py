...
def evaluate_motor_health(residuals):
    motor_health = residuals[['Actual Roll', 'Actual Pitch', 'Actual Yaw']].std().rename({
        'Actual Roll': 'Motor 1 Health',
        'Actual Pitch': 'Motor 2 Health',
        'Actual Yaw': 'Motor 3 Health'
    })
    motor_health['Motor 4 Health'] = motor_health.mean()  # Example: average of other motors' health as a placeholder
    return motor_health

def plot_motor_health(motor_health):
    motor_health.plot(kind='bar', color=['blue', 'green', 'red', 'purple'])
    plt.title('Motor Health Summary')
...
def main():
    ...
    if data is not None:
        ...
        motor_health = evaluate_motor_health(residuals)
        plot_motor_health(motor_health)
