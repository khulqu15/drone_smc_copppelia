...
def generate_statistics(data, motor_faults, time_step=None):
    stats = {
        'Motor': [],
        'State': [],
        'φ (deg)': [],
    ...
    if time_step is None:
        try:
            time_step = 1 / data.index.freqstr
        except AttributeError:
            time_step = 1  # Set a default time_step if none is provided

    for motor, faults in motor_faults.items():
        is_faulty = faults.any().any()
        stats['Motor'].append(motor)
        stats['State'].append('LOE' if is_faulty else 'Healthy')
        
        φ = data['Roll'].mean()
        θ = data['Pitch'].mean()
        ze = data['Altitude'].mean() if 'Altitude' in data.columns else 0
        
        p = φ / time_step
        q = θ / time_step

        # Calculate RPM only for Roll as an example
        roll_changes = np.diff(data['Actual Roll'].dropna()) / time_step
        rpm = np.mean(np.abs(roll_changes)) / 360 * 60  # Convert angular velocity to RPM

        stats['φ (deg)'].append(φ)
        stats['θ (deg)'].append(θ)
        stats['ze (m)'].append(ze)
        stats['p (deg/s)'].append(p)
        stats['q (deg/s)'].append(q)
        stats['RPM'].append(rpm if not np.isnan(rpm) else 0)
...
def main():
    ...
    if data is not None:
        ...
        statistics = generate_statistics(stationary_data, motor_faults, time_step=0.1)
        print(statistics)
