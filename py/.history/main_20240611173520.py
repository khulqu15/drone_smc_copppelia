...
def main():
    ...
    if data is not None:
        ...
        actuals = ['Actual Roll', 'Actual Pitch', 'Actual Yaw', 'Actual Z']
        
        for var, act in zip(variables, actuals):
            temp_data = data[[var, act]].dropna()
        ...
        for i, ax in enumerate(fig.get_axes()):
            ax.set_title(labels[i % 8] + ' to ' + labels[i // 8])
        plt.show()
