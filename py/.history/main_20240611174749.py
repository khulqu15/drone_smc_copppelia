...
def main():
    ...
    if data is not None:
        ...
        for i, var in enumerate(variables):
            ax = axes[i//2, i%2]
            irf.plot(impulse=var, response='Actual ' + var, ax=ax, plot_stderr=False)
            ax.set_title(f'Impulse Response of {var} to Actual {var}')
            ax.legend([f'IRF of {var} to Actual {var}'])
