...
def simulate_motor_health(data, residuals):
    # Mengasumsikan keempat motor memiliki kontribusi yang berbeda pada kesalahan total
    motor_weights = np.random.rand(4, residuals.shape[1])  # Bobot random untuk 4 motor dan 3 orientasi
    motor_weights /= motor_weights.sum(axis=0)  # Normalisasi bobot sehingga sum per kolom menjadi 1

    motor_health = {}
    for i in range(1, 5):
        motor_health['Motor' + str(i)] = (residuals * motor_weights[i-1]).abs().mean(axis=1)
    
    motor_health['Healthy'] = residuals.abs().mean(axis=1) * 0.5  # Asumsikan motor sehat memiliki setengah rata-rata kesalahan
