...
def make_stationary(data):
    ...
    for column in non_stationary:
        data.loc[:, column] = data.loc[:, column].diff().fillna(0)  # Use loc for safe in-place modification
    return data
...
def main():
    ...
    if data is not None:
        ...
        # Making only the necessary columns stationary
        stationary_data = make_stationary(data[['Actual X', 'Actual Y', 'Actual Z', 'Actual Roll', 'Actual Pitch', 'Actual Yaw']].copy())
